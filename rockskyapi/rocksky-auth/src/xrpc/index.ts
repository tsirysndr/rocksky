...
export default function (server: Server) {
  ...
  getAlbum(server);
  getAlbums(server);
  getAlbumTracks(server);
  createApikey(server);
  getApikeys(server);
  removeApikey(server);
  updateApikey(server);
  getArtist(server);
  getArtistAlbums(server);
  getArtists(server);
  getArtistTracks(server);
  getScrobblesChart(server);
  downloadFileFromDropbox(server);
  getFilesFromDropbox(server);
  getMetadataFromDropbox(server);
  getTemporaryLinkFromDropbox(server);
  getNowPlayings(server);
  search(server);
  downloadFileFromGoogleDrive(server);
  getFilesFromGoogleDrive(server);
  getFileFromGoogleDrive(server);
  dislikeShout(server);
  dislikeSong(server);
  likeShout(server);
  likeSong(server);
  getCurrentlyPlaying(server);
  next(server);
  pause(server);
  previous(server);
  seek(server);
  getPlaylist(server);
  getPlaylists(server);
  createScrobble(server);
  getScrobble(server);
  getScrobbles(server);
  createShout(server);
  getAlbumShouts(server);
  getArtistShouts(server);
  getProfileShouts(server);
  getTrackShouts(server);
  removeShout(server);
  replyShout(server);
  reportShout(server);
  getSong(server);
  getSongs(server);
  spotifyGetCurrentlyPlaying(server);
  spotifyNext(server);
  spotifyPause(server);
  spotifyPrevious(server);
  spotifySeek(server);
  spotifyPlay(server);
  getStats(server);
