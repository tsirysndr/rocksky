...
function detectLightTerminal(): boolean {
  ...
  if (process.stdout.isTTY) {
    try {
      const savedState = execSync("stty -g </dev/tty 2>/dev/null", {
        encoding: "utf8",
      }).trim();
...
// Resolved on first use, not at import: the OSC 11 probe below puts the tty in
// raw mode briefly, and the TUI imports this module only for the hex palette.
let lightTerminal: boolean | null = null;
function isLightTerminal(): boolean {
  if (lightTerminal === null) lightTerminal = detectLightTerminal();
  return lightTerminal;
}

/** Hex form of the palette, for consumers that need a colour string (Ink). */
export const PRIMARY = "#00E8C6";
export const SECONDARY = "#00C6E8";
export const ACCENT = "#8264FF";
export const HIGHLIGHT = "#64E882";
export const MUTED = "#C8D2DC";
export const LINK = "#FFA064";
export const SKY = "#00D2FF";
export const ERROR = "#FF6464";
...
export const c = {
  ...
  muted: (s: string | number) =>
    isLightTerminal() ? chalk.black(s) : chalk.rgb(200, 210, 220)(s),
