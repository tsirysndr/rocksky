...
export default function (server: Server, ctx: Context) {
  const config = {
    ...
    handler: async (reqCtx: DeleteReqCtx) => {
      ...
      if (!id) {
        return { status: 400, message: "Missing album id." };
      }

      try {
        const deleted = await deleteUploadsForAlbumId(ctx, did, id);
        if (deleted === 0) {
          return {
            status: 404,
            message: "No uploaded songs found for the album.",
          };
        }
        return {
        ...
        consola.error("[library.deleteAlbum]", err);
        return { status: 500, message: "Internal error while deleting album." };
      }
    },
  };
  ...
  server.app.rocksky.library.deleteAlbum(config as unknown as never);
}
