...
const retrieve = ({
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      const albumIds = artistAlbumRows.map((r) => r.albumId).filter((id): id is string => id !== null);
      ...
      const playCountMap = new Map(scrobbleCounts.map((r) => [r.albumId, Number(r.play_count)]));
