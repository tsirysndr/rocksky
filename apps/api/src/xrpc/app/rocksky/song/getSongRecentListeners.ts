...
export default function (server: Server, ctx: Context) {
  ...
  const getSongRecentListeners = (params: QueryParams) =>
    pipe(
      cache,
  ...
  server.app.rocksky.song.getSongRecentListeners({
    handler: async ({ params }) => {
      const result = await Effect.runPromise(getSongRecentListeners(params));
      return {
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      const scrobbleRows = await ctx.db
        ...
        .where(
          and(
            eq(tables.scrobbles.trackId, track.id),
      ...
      new Error(`Failed to retrieve song's recent listeners: ${error}`),
  });
};
