...
export function isPlayView(v: unknown): v is PlayView {
  return (
    isObj(v) && hasProp(v, "$type") && v.$type === "fm.teal.feed.defs#playView"
  );
}

export function validatePlayView(v: unknown): ValidationResult {
  return lexicons.validate("fm.teal.feed.defs#playView", v);
}
...
export function isArtist(v: unknown): v is Artist {
  return (
    isObj(v) && hasProp(v, "$type") && v.$type === "fm.teal.feed.defs#artist"
  );
}

export function validateArtist(v: unknown): ValidationResult {
  return lexicons.validate("fm.teal.feed.defs#artist", v);
}
