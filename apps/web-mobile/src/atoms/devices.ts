...
export type RemoteDevice = {
  ...
  // pushes so the player screen's queue panel stays in sync.
  queue: QueueTrack[];
  queueIndex: number;
};

// Every connected player device, keyed by device_id — the source of truth for
// the source sheet. A user can have several players streaming at once; each gets
// its own entry, so their states never conflict.
...
// Bridge so library context menus can enqueue on the active remote device
// without owning the WebSocket. MiniPlayer publishes it; `active` is true when a
// remote device is the current player.
