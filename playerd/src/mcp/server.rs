...
Control Rocksky playback: real speakers, real audio, right now.

`list_devices` shows every player online for this account (playerd daemons, the \
desktop app, web miniplayers); every command tool takes an optional `device` \
(name or id) and picks the obvious one when there is only one.
