...
impl Player {
    ...
    pub fn connect(config: &Config, token: String) -> std::sync::Arc<Self> {
        let controller = RemoteController::connect(
            RemoteControllerConfig::new(token, "playerd-mcp").url(config.ws_url.clone()),
        );
    ...
    pub fn resolve(&self, arg: Option<&str>) -> Result<Target> {
        ...
        let wanted = arg
            ...
            .filter(|s| !s.is_empty())
            .map(str::to_string)
            .or_else(|| std::env::var("PLAYERD_MCP_DEVICE").ok())
        ...
        if devices.is_empty() {
            bail!("no players are online. Start `playerd` (or open the Rocksky web/desktop app) and try again");
        }
        bail!(
