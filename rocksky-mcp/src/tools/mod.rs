//! The tool surface: everything an agent can do with a Rocksky account.
//!
...
//! [`insights`] answers questions about the account and the platform. The
//! first three are thin wrappers over the same remote-control protocol the web
//! and desktop miniplayers speak, so anything the agent does here is visible
//! (and reversible) from any other Rocksky client.
...
use crate::rocksky::{Me, Rocksky};
use crate::state::Player;
use crate::subsonic::Subsonic;

/// How long to wait after a transport command before reading the state back.
/// A player re-pushes ~150 ms after applying one; the rest is the round trip.
...
impl Ctx {
    ...
    pub async fn subsonic(&self) -> Result<&Arc<Subsonic>> {
        self.subsonic
            .get_or_try_init(|| async {
                Subsonic::connect(&self.config.api_url, self.rocksky.token())
                    .await
                    .map(Arc::new)
            })
            .await
    ...
    /// The actor a tool call is about: the `actor` argument (a handle or DID)
    /// if it has one, otherwise the logged-in account.
    pub async fn actor(&self, args: &Args<'_>) -> Result<String> {
        match args.str("actor") {
            Some(actor) => Ok(actor.to_string()),
            None => Ok(self.me().await?.did.clone()),
        }
    }
}
...
pub fn definitions() -> Vec<Value> {
    ...
    tools.extend(insights::definitions());
    tools
}
...
pub async fn call(ctx: &Ctx, name: &str, args: &Value) -> Result<Value> {
    ...
    if let Some(result) = insights::call(ctx, name, &args).await? {
        return Ok(result);
    }
...
/// The `actor` argument, shared by every tool that reads a public profile.
pub fn actor_prop() -> Value {
    json!({
        "type": "string",
        "description": "Whose data to read: a Rocksky/Bluesky handle (alice.bsky.social) or a DID. Omit for the logged-in account.",
    })
}
