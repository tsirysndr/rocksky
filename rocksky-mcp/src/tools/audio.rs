...
use crate::settings::{self, PresetSpec, EQ_BAND_FREQUENCIES};
...
pub async fn call(ctx: &Ctx, name: &str, args: &Args<'_>) -> Result<Option<Value>> {
    let result = match name {
        "get_audio_settings" => {
            let me = ctx.me().await?;
            let saved = ctx.rocksky.audio_settings(&me.did).await?;
            match saved {
                Some(view) => json!({
        ...
        "list_equalizer_presets" => {
            let presets = ctx.rocksky.equalizer_presets(None).await?;
            json!({
                "presets": presets
                    .iter()
                    .map(|p| json!({
        ...
        "apply_equalizer_preset" => {
            ...
            let presets = ctx.rocksky.equalizer_presets(did.as_deref()).await?;
            let preset = settings::find_preset(&presets, &spec)?;
            let document = RemoteAudioSettings {
                equalizer: Some(RemoteEqualizer {
                    enabled: Some(true),
                    precut: preset.precut.map(|p| p as i32),
                    bands: Some(
                        preset
                            .bands
                            .iter()
                            .map(|b| RemoteEqBand {
                                frequency: b.frequency as i32,
                                gain: b.gain as i32,
                                q: b.q as i32,
                            })
                            .collect(),
