...
function AddToPlaylistPalette() {
  const [song, setSong] = useAtom(addToPlaylistSongAtom);
  const profile = useAtomValue(profileAtom);
  const setCreateOpen = useSetAtom(createPlaylistModalOpenAtom);
  const setSeedSongs = useSetAtom(newPlaylistSeedSongsAtom);
  ...
  const { data: playlists = [] } = usePlaylistsQuery(profile?.did ?? "");

  const matches = useMemo(() => {
    const q = query.trim().toLowerCase();
    const own = playlists.filter((p) => !!p.uri);
    if (!q) return own;
    return own.filter((p) =>
  ...
  const startCreate = () => {
    setSeedSongs([song.uri]);
    setSong(null);
    setCreateOpen(true);
  };

  const addTo = async (playlistUri: string) => {
    if (added.has(playlistUri) || pending) return;
    setPending(playlistUri);
    setError(null);
    try {
      await addSongs.mutateAsync({ uri: playlistUri, songs: [song.uri] });
      setAdded((prev) => new Set(prev).add(playlistUri));
    } catch (e) {
      setError(
  ...
  const choose = (index: number) => {
    ...
    if (playlist?.uri) void addTo(playlist.uri);
  };
        ...
        {matches.length > 0 && (
          <Results>
            {matches.map((playlist, idx) => {
              const isAdded = !!playlist.uri && added.has(playlist.uri);
              return (
                <Row
                  key={playlist.id}
                  active={active === idx + 1}
                ...
                >
                  ...
                  {isAdded && <IconCheck size={16} />}
                  {pending === playlist.uri && <Secondary>Adding…</Secondary>}
                </Row>
              );
            })}
          </Results>
...
export default AddToPlaylistPalette;
