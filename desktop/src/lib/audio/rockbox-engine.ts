...
// Same tracks keyed by uploadId: stream URLs carry a rotating ?token=, so an
// exact-URL lookup misses after a token refresh (or, on desktop, a webview
// reload that outlives the native engine). The uploadId in the path is stable.
const registryById = new Map<string, QueueTrack>();
