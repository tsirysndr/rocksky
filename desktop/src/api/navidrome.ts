...
function sr(data: unknown) {
  return (data as Record<string, unknown>)["subsonic-response"] as Record<
    string,
    unknown
  >;
}
...
export function getNavidromeStreamUrl(
  creds: NavidromeCredentials,
  songId: string,
): string {
...
export async function fetchNavidromeAlbums(
  ...
  return (
    (sr(res.data)?.albumList2 as { album?: NavidromeAlbum[] })?.album ?? []
  );
}
...
export async function fetchNavidromeArtists(
  ...
  const indexes =
    (sr(res.data)?.artists as { index?: { artist: NavidromeArtist[] }[] })
      ?.index ?? [];
...
export async function fetchNavidromeAlbum(
  ...
  const res = await axios.get(restUrl("getAlbum"), {
    params: qp(creds, { id: albumId }),
  });
...
export async function fetchNavidromeArtist(
  ...
  const res = await axios.get(restUrl("getArtist"), {
    params: qp(creds, { id: artistId }),
  });
...
export async function fetchNavidromePlaylists(
  ...
  return asArray<NavidromePlaylist>(
    (
      sr(res.data)?.playlists as {
        playlist?: NavidromePlaylist | NavidromePlaylist[];
      }
    )?.playlist,
  );
}
...
export async function fetchNavidromePlaylist(
  ...
  const res = await axios.get(restUrl("getPlaylist"), {
    params: qp(creds, { id }),
  });
...
export async function setNavidromePlaylistDescription(
  ...
  throwIfMirrorFailed(
    await rocksky()
      .library()
      .updatePlaylist(playlistId, { comment: description }),
  );
}
