...
# of the shared Rust core (`rocksky-uniffi` → `rocksky-sdk`). This is the write +
# dedup side (AT Protocol PDS writes); the HTTP client in the top-level `Rocksky`
# module is the read side. Auth / record-write logic is identical to the Rust,
# Python, Clojure, and BEAM SDKs.
require "fiddle"
require "fiddle/import"
require "json"
require_relative "core/native"
  ...
  module Core
    ...
    class Agent
      def self.login(session_path, identifier, password, appview: nil)
        ...
        raise Error, (Core.take_string(C.rocksky_last_error()) || "login failed") if ptr.null?
      ...
      def scrobble(track)
        Core.unwrap(C.rocksky_agent_scrobble(@ptr, JSON.generate(track)))
      end

      def like(uri, cid)
        Core.unwrap(C.rocksky_agent_like(@ptr, uri, cid))
      end

      def follow(did)
        Core.unwrap(C.rocksky_agent_follow(@ptr, did))
      end

      def shout(subject_uri, subject_cid, message)
        Core.unwrap(C.rocksky_agent_shout(@ptr, subject_uri, subject_cid, message))
      end

      def refresh_session
        Core.unwrap(C.rocksky_agent_refresh_session(@ptr))
      end
