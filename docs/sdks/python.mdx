...
<CodeGroup>
```bash uv
uv add rocksky==0.6.0
```

```bash pip
pip install rocksky==0.6.0
```
</CodeGroup>
...
`scrobble_match(ScrobbleMatchInput(title, artist, album=None, mb_id=None,
isrc=None, timestamp=None))` (resolve via `match_song`, then fan out — `title`/
