...
`fm.teal.feed.play` records. Rocksky also accepts legacy
`fm.teal.alpha.feed.play` records during the namespace migration. No polling,
no API keys — your ATProto DID is the only identifier needed.
...
- One process-wide WebSocket connection subscribes to both
  `fm.teal.feed.play` and legacy `fm.teal.alpha.feed.play` on Jetstream.
