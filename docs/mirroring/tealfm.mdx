...
`fm.teal.feed.play` (and the legacy `fm.teal.alpha.feed.play`). No polling,
no API keys — your ATProto DID is the
...
- One process-wide WebSocket connection subscribes to
  both `fm.teal.feed.play` and `fm.teal.alpha.feed.play` on Jetstream.
