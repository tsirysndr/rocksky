...
CREATE TABLE IF NOT EXISTS tracks (
  ...
  mb_id              TEXT,
  isrc               TEXT,
  -- The streaming links are deliberately *not* unique here, though
  -- apps/api's Drizzle schema declares them so.
  --
  -- A `tracks` row is a recording *in an album context* — its sha256 is
  -- `title - artist - album` — while a streaming link identifies the
  -- recording alone. So "Somewhere Only We Know" on `Hopes And Fears` and on
  -- a `Now That's What I Call Music` compilation are two rows sharing one
  -- Spotify URL, by design.
  --
  -- Production agrees with the data rather than with its own schema: its
  -- `tracks.spotify_link` has 16,167 duplicate values and no unique index.
  -- The other three columns are declared unique there and satisfy it only
  -- because nothing has ever populated them. Copying the declaration cost 10
  -- dropped records on the first real backfill.
  --
  -- `albums` keeps its unique links: two editions of a release have two
  -- distinct album URLs, so those really are one-to-one.
  youtube_link       TEXT,
  spotify_link       TEXT,
  apple_music_link   TEXT,
  tidal_link         TEXT,
  -- The identity, and the one that must stay unique.
  sha256             TEXT NOT NULL UNIQUE,
  disc_number        INTEGER,
  lyrics             TEXT,
  composer           TEXT,
