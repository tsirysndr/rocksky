...
pub trait Feed<Handler: FeedHandler + Clone + Send + Sync + 'static> {
    ...
    fn start_with_config(
        ...
        async move {
            ...
            let config = config;
            let pg = RepoImpl::Postgres(PostgresRepo::new().await?);
            let pg_filter = warp::any().map(move || pg.clone());
            ...
            let describe_feed_generator = warp::path("xrpc")
                ...
                .and(warp::get())
                .and(pg_filter.clone())
                .and_then(move |_repo: RepoImpl| describe_feed_generator(feed_name.clone()));
            ...
            let get_feed_skeleton = warp::path("xrpc")
                ...
                .and(pg_filter.clone())
                .and_then(move |query: FeedSkeletonParameters, repo: RepoImpl| {
                    get_feed_skeleton::<Handler>(query.into(), get_feed_handler.clone(), repo)
                });
