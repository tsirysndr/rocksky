...
/** Response marker for the `app.rocksky.actor.getActorPlaylists` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetActorPlaylistsOutput<S>` for this endpoint.*/
...
impl jacquard_common::xrpc::XrpcResp for GetActorPlaylistsResponse {
    ...
    type Output<S: BosStr> = GetActorPlaylistsOutput<S>;
    type Err = jacquard_common::xrpc::GenericError;
}

impl<S: BosStr> jacquard_common::xrpc::XrpcRequest for GetActorPlaylists<S> {
    const NSID: &'static str = "app.rocksky.actor.getActorPlaylists";
    const METHOD: jacquard_common::xrpc::XrpcMethod = jacquard_common::xrpc::XrpcMethod::Query;
    type Response = GetActorPlaylistsResponse;
}

/** Endpoint marker for the `app.rocksky.actor.getActorPlaylists` query.

Path: `/xrpc/app.rocksky.actor.getActorPlaylists`. The request payload type is `GetActorPlaylists<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
impl jacquard_common::xrpc::XrpcEndpoint for GetActorPlaylistsRequest {
    ...
    const METHOD: jacquard_common::xrpc::XrpcMethod = jacquard_common::xrpc::XrpcMethod::Query;
    type Request<S: BosStr> = GetActorPlaylists<S>;
    type Response = GetActorPlaylistsResponse;
}
...
impl<S: BosStr> GetActorPlaylists<S> {
    /// Create a new builder for this type
    pub fn builder() -> GetActorPlaylistsBuilder<get_actor_playlists_state::Empty, S> {
        GetActorPlaylistsBuilder::builder()
    }
}

impl GetActorPlaylistsBuilder<get_actor_playlists_state::Empty, DefaultStr> {
...
impl<S: BosStr> GetActorPlaylistsBuilder<get_actor_playlists_state::Empty, S> {
...
impl<St, S: BosStr> GetActorPlaylistsBuilder<St, S>
where
    St: get_actor_playlists_state::State,
    St::Did: get_actor_playlists_state::IsUnset,
{
    /// Set the `did` field (required)
    pub fn did(
        mut self,
        value: impl Into<AtIdentifier<S>>,
    ) -> GetActorPlaylistsBuilder<get_actor_playlists_state::SetDid<St>, S> {
...
where
    St: get_actor_playlists_state::State,
    St::Did: get_actor_playlists_state::IsSet,
{
