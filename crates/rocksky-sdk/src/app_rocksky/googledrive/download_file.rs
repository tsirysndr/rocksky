...
/** Response marker for the `app.rocksky.googledrive.downloadFile` query.
...
/** Endpoint marker for the `app.rocksky.googledrive.downloadFile` query.

Path: `/xrpc/app.rocksky.googledrive.downloadFile`. The request payload type is `DownloadFile<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
