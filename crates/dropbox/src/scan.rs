...
pub fn scan_audio_files(
    ...
    Box::pin(async move {
        ...
        let res = client
            .post(&format!("{}/files/get_metadata", BASE_URL))
        ...
        if entry.tag.clone().unwrap().as_str() == "folder" {
            ...
            let res = client
                .post(&format!("{}/files/list_folder", BASE_URL))
                .bearer_auth(&access_token)
                .json(&json!({ "path": path }))
                .send()
                .await?;
            ...
            while entry_list.has_more {
                let res = client
                    .post(&format!("{}/files/list_folder/continue", BASE_URL))
                    .bearer_auth(&access_token)
        ...
        let hash = sha256::digest(format!("{} - {} - {}", title, artist, album).to_lowercase());
        ...
        match track {
            Some(track) => {
                ...
                let status = create_dropbox_path(&pool, &entry, &track, &dropbox_id).await;
          ...
          .await?;
                ...
                if let Some(track) = track {
                    create_dropbox_path(&pool, &entry, &track, &dropbox_id).await?;
                    return Ok(());
                }
