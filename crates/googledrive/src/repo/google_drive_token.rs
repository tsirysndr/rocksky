...
pub async fn find_google_drive_refresh_token(
    pool: &Pool<Postgres>,
    did: &str,
) -> Result<Option<(String, String)>, Error> {
    let results: Vec<GoogleDriveTokenWithDid> = sqlx::query_as(
        r#"
    SELECT
      gd.xata_id,
      gd.xata_version,
...
pub async fn find_google_drive_refresh_tokens(
    pool: &Pool<Postgres>,
) -> Result<Vec<GoogleDriveTokenWithDid>, Error> {
    let results: Vec<GoogleDriveTokenWithDid> = sqlx::query_as(
        r#"
    SELECT
      gd.xata_id,
