...
pub async fn get_albums(
    ...
    tracing::info!(limit, offset, user_did = ?did, genre = ?genre, "Get albums");
    ...
    match did {
      ...
      "#,
            ...
            let albums_iter = stmt.query_map(duckdb::params_from_iter(params), |row| {
                Ok(Album {
                    id: row.get(0)?,
                    title: row.get(1)?,
                    artist: row.get(2)?,
