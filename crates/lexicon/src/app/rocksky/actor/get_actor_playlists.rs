...
pub const NSID: &str = "app.rocksky.actor.getActorPlaylists";
...
pub struct Parameters {
    ...
    /// createdAt, updatedAt. The `track.title`, `track.artist`, `track.album`
    /// and `track.albumArtist` selectors match the playlist's contents,
    /// returning playlists that contain a matching track; several `track.*`
    /// terms joined with `;` must all be satisfied by the same track.
