//! The ATProto protocol surface Rocksky needs.
//!
//! Four things, none of which involve a database or an HTTP server, which is
//! why they are their own crate:
//!
//! | module          | what it does                                      |
//! |-----------------|---------------------------------------------------|
//! | [`car`]         | reads the CAR v1 archive `getRepo` returns        |
//! | [`mst`]         | walks the Merkle Search Tree for record keys      |
//! | [`records`]     | writes `app.rocksky.*` records to a repository    |
//! | this module     | resolves a DID to its PDS, and fetches a repo     |
//!
//! Resolution is done directly over HTTP rather than through
//! `jacquard-identity`, so a caller decides where its DID cache lives and the
//! sync path has one fewer moving part.
//!
//! Session *storage* is deliberately not here: where an instance keeps its
//! sessions is its own concern, so [`AtpSession`] carries only the credentials
//! a write needs and the appview owns the table they come from.
...
/// The credentials needed to write to a repository.
///
/// An app-password session, reduced to what [`records`] uses. The full session
/// a PDS hands back has an email, a refresh token and an active flag; none of
/// them matter to a `putRecord`.
#[derive(Debug, Clone)]
