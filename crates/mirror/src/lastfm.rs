...
// "now" (which would skip every scrobble that happened before this task booted).
...
pub async fn run_user(
    ...
    loop {
        ...
        tokio::select! {
            _ = cancel.cancelled() => {
                info!(user_id = %row.user_id, "Last.fm: cancelled mid-poll");
                return Ok(());
            }
            result = poll => match result {
                ...
                Err(_) => {
                    error!(
                        ...
                        "Last.fm: poll_once timed out, will retry next interval"
                    );
                }
            }
        }
...
/// One poll of Last.fm for one user — this service's unit of work, so it is
/// also the span. `skip_all` because the arguments are connection pools and
