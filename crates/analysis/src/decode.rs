...
pub struct Decoded {
    ...
    /// key. A window, not the whole track: those detectors are quadratic-ish
...
pub fn decode(bytes: &[u8], extension_hint: Option<&str>) -> Result<Decoded> {
    ...
    // Not known until the first decoded packet for some codecs, so this starts
    // empty and is filled in on the way.
    ...
    decoded.duration = if stopped_early {
        claimed_duration.unwrap_or(frames_seen as f32 / sample_rate)
    } else {
        frames_seen as f32 / sample_rate
    };
